...
client.on('message', message =>{
    ...
    let prefix = 's!'; //تقدر تغير البرفكس
...
client.on("guildMemberAdd", async member => {
  let welcomechannelsd = member.guild.channels.find(c => c.name === "𝐿𝑒𝑔𝑒𝓃𝒹𝒶𝓇𝓎")
  welcomechannelsd.send(`(<@${member.id}>) Welcome to server \`~ (${member.guild.name})\``)
});
